...
def switch_flat(
    ...
    The parameter_trios is list or tuple starts with a first value called switch or switch value.
    -
    
    After the switch follows one or more trios and/or pairs of values.
    For each trio encountered:
        The trio has a case [case] (possible value for switch), a function/callable [fxn] (called on switch==case), and arguments [args] (fxn([*[*]]args)).
    ...
    If parameter_trios is empty, the base case is invoked (if given) and then the error is called (error[0](*error[1])), if given.
    -
