...
class MoneyParsingErrors(Enum) :
    ...
    def is_error(x) -> bool :
        ...
        """
        if isinstance(x, int) or isinstance(x, float) : return False
        if isinstance(x, MoneyParsingErrors) :
            if x == MoneyParsingErrors.NO_ERROR : return False
            return True
        return False
...
class PercentageParsingErrors(Enum) :
    ...
    def is_error(x) -> bool :
        ...
        if isinstance(x, PercentageParsingErrors) :
            if x == PercentageParsingErrors.NO_ERROR : return False
            return True
        return False
...
def is_money(string: str) :
    """
    Take a formal dollar-value string string and, if possible, return the dollar value in decimal that it represents.  If not, return MoneyParsingErrors.
    ...
    if len(temporary) < 6 : return MoneyParsingErrors.TOO_SHORT  # "+$0.00" has 6 characters
    if temporary[-1] not in ('+', '-') : return MoneyParsingErrors.SIGNAGE_MISSING  # verify intact signage
    negative_v: bool = True  # negative until positive
    if temporary.pop(-1) == '+' : negative_v = False  # if it's positive, it's not negative
    if temporary.pop(-1) != '$' : return MoneyParsingErrors.DOLLAR_SIGN_MISSING  # require $ after signage
    if temporary[2] != '.' : return MoneyParsingErrors.DECIMAL_POINT_MISSING  # decimal point after cents
    if not temporary[0] in digits : return MoneyParsingErrors.FIRST_DIGIT_MALFORMED  # last digit of cents
    if not temporary[1] in digits : return MoneyParsingErrors.SECOND_DIGIT_MALFORMED  # first digit of cents
    ...
    if negative_v : balance *= -1  # restore signage
    return balance
...
def is_percentage(string: str) :
    """
    Take a formal percentage-value string string and, if possible, return the percentage value in decimal that it represents.  If not, return PercentageParsingErrors.
    Format description from left to right:
        The format starts with a mandatory signage character (either "+" or "-").
    ...
    """
    if len(string) < 5 : return PercentageParsingErrors.TOO_SHORT  # +0.0% has 5 characters
    if string[0] not in ('-', '+') : return PercentageParsingErrors.SIGNAGE_MISSING
    if string[-1] != '%' : return PercentageParsingErrors.PERCENT_SIGN_MISSING
    if '.' not in string : return PercentageParsingErrors.DECIMAL_POINT_MISSING
    try :
