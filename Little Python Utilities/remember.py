...
class Memory(object) :
    ...
    def __del__(self) :
        ...
        if self.is_active() : self.remember()
    
    def __eq__(self, o) :
        """Returns True if "o" is Equal to the piece of data being held by this API."""
        
        if not self.is_active() : raise LoadingError("Please initialize this API.")
        try :
    ...
    def __iter__(self) :
        """When the class API is iterated over, it yields back through the current piece of data being held in this API."""
    ...
    # noinspection PyIncorrectDocstring,PyIncorrectDocstring,PyIncorrectDocstring,PyIncorrectDocstring,PyIncorrectDocstring,PyIncorrectDocstring,PyIncorrectDocstring
    ...
    def target(self, target, save=True) :
        ...
        ty = 0
        if save :
            if self.get_target() not in (None, "") : ty = 1 + self.remember()
    ...
    def set_type_enforcement_state(self, true) :
        ...
        if true : return self.set_type(self.get_type())
        return 0
    ...
    def remember(self) :
        """This method saves the current piece of data that this currently being held in this API to the document that this currently being targeted by this API."""
